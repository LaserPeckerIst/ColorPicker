...
public class ColorPanelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (shape == ColorShape.SQUARE) {
            ...
            int cx = drawWidth / 2 + getPaddingLeft();
            int cy = drawHeight / 2 + getPaddingTop();
            final int outerRadius = Math.min(drawWidth, drawHeight) / 2;
            if (borderWidthPx > 0) {
                canvas.drawCircle(cx, cy, outerRadius, borderPaint);
            }
            if (Color.alpha(color) < 255) {
                canvas.drawCircle(cx, cy, outerRadius - borderWidthPx, alphaPaint);
            }
            if (showOldColor) {
                canvas.drawArc(centerRect, 90, 180, true, originalPaint);
                canvas.drawArc(centerRect, 270, 180, true, colorPaint);
            } else {
                canvas.drawCircle(cx, cy, outerRadius - borderWidthPx, colorPaint);
            }
        }
    }
